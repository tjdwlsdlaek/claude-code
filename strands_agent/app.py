"""FastAPI application for Strands Agents real estate advisor."""
...
import logging
import uvicorn
from workflow import run_real_estate_workflow
...
# Create FastAPI app
app = FastAPI(
    title="Strands Agents Real Estate Advisor",
    description="AI agent system for real estate purchase strategy using Strands pattern",
    version="1.0.0"
)
...
async def root():
    """Root endpoint."""
    return {
        "service": "Strands Agents Real Estate Advisor",
        "status": "running",
        "framework": "strands",
        "version": "1.0.0"
    }
...
async def analyze_real_estate(query: RealEstateQuery):
    ...
    try:
        ...
        # Run workflow
        result = run_real_estate_workflow(query_dict)
...
async def test_with_example():
    ...
    try:
        result = run_real_estate_workflow(example_query)
        return result
    except Exception as e:
...
if __name__ == "__main__":
    ...
    uvicorn.run(
        "app:app",
        host="0.0.0.0",
        port=settings.app_port + 2,  # Different port from others
        reload=True,
