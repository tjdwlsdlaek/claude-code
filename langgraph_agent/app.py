"""FastAPI application for LangGraph real estate advisor."""
...
import logging
import uvicorn
from graph import run_real_estate_advisor
...
# Create FastAPI app
app = FastAPI(
    title="LangGraph Real Estate Advisor",
    description="AI agent system for real estate purchase strategy using LangGraph",
    version="1.0.0"
)
...
async def root():
    """Root endpoint."""
    return {
        "service": "LangGraph Real Estate Advisor",
        "status": "running",
        "framework": "langgraph",
        "version": "1.0.0"
    }
...
async def analyze_real_estate(query: RealEstateQuery):
    ...
    try:
        ...
        # Run workflow
        result = run_real_estate_advisor(query_dict)
...
async def test_with_example():
    ...
    try:
        result = run_real_estate_advisor(example_query)
        return result
    except Exception as e:
